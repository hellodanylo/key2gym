...
public class ManageReportsDialog extends AbstractDialog {
    ...
    private void initComponents() {
        ...
        openButton.setIcon(new ImageIcon(getClass().getResource("/org/key2gym/client/resources/document32.png"))); // NOI18N
        ...
        saveButton.setIcon(new ImageIcon(getClass().getResource("/org/key2gym/client/resources/save32.png"))); // NOI18N
    ...
    private void buildDialog() {
        ...
        {
            buttonsPanel.setLayout(new FormLayout("d", "b:d:g, c:d, c:d, c:d, t:d:g, d"));
	...
	PickReportFormatDialog reportFormatDialog = new PickReportFormatDialog(this, report);
	...
	/*
	 * If the required format is not available,
	 * converts the report to it right now.
	 */
	if(!report.getFormats().contains(reportFormat)) {
	    try {
		service.convertReport(report.getId(), reportFormat);
	    } catch(ValidationException ex) {
		throw new RuntimeException("The report's id should have been valid", ex);
	    } catch(SecurityViolationException ex) {
		UserExceptionHandler.getInstance().processException(ex);
		return;
	    }
	...
	try {
	    reportHandlerDialog = new PickReportHandlerDialog(this, reportFormat);
	} catch(UserException ex) {
	    UserExceptionHandler.getInstance().processException(ex);
	    return;
	}
	...
	reportHandlerDialog.getReportHandler().handle(report, reportBody, reportFormat);
	...
	try {
	    ...
	    chooser.setSelectedFile(new java.io.File(System.getProperty("user.home") + "/key2gym-report-" + report.getId() + "." + reportFormat));
