 ...
 */
@Stateless
@Remote(FreezesServiceRemote.class)
...
public class FreezesServiceBean extends BasicBean implements FreezesServiceRemote {
    ...
    public void addFreeze(Integer clientId, Integer days, String note) throws ValidationException, BusinessException, SecurityViolationException {
        ...
        AdministratorsServiceRemote administratorsService;
        try {
            administratorsService = InitialContext.doLookup("java:module/AdministratorsServiceBean");
        } catch (NamingException ex) {
            throw new EJBException(ex);
        }
        clientFreeze.setAdministrator(getEntityManager().find(Administrator.class, administratorsService.getByUsername(getSessionContext().getCallerPrincipal().getName()).getId()));
